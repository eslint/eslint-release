...
function validateSetup() {
	...
	if (!pkg.repository) {
		...
		console.error(
			"The 'repository' field must be in the format 'foo/bar' in package.json",
		);
		ShellOps.exit(1);
	}
}
...
function calculateReleaseFromGitLogs(currentVersion, logs, prereleaseId) {
	...
	function generateChangelogLine(log) {
		return log.raw.replace(
			/^\* ([0-9a-f]{40})/,
			(_, hash) =>
				`* [\`${hash.slice(0, 7)}\`](https://github.com/${repository}/commit/${hash})`,
		);
	}
	const releaseInfo = {
	...
	releaseInfo.version = prereleaseId
...
function calculateReleaseInfo(prereleaseId) {
	...
	// get log statements
	const logs = ShellOps.execSilent(
		`git log --no-merges --pretty=format:"* %H %s (%an)%n%b" ${commitRange}`,
	).split(/\n/g);
	const releaseInfo = calculateReleaseFromGitLogs(
		pkg.version,
		logs,
		prereleaseId,
	);
...
function writeChangelog(releaseInfo) {
	...
	fs.writeFileSync("CHANGELOG.tmp", releaseInfo.markdownChangelog);
...
function generateRelease(
	prereleaseId,
	packageTag = prereleaseId ? "next" : "latest",
) {
	validateSetup();
	...
	ShellOps.exec(
		`git commit -m "Build: changelog update for ${releaseInfo.version}"`,
	);
	...
	getPackageInfo()
		.files.filter(dirPath => fs.lstatSync(dirPath).isDirectory())
		.forEach(filePath => {
	...
	fs.writeFileSync(
		...
		JSON.stringify(releaseInfo, null, 4),
	);
}
...
function publishRelease() {
	...
	const releaseInfo = JSON.parse(
		fs.readFileSync(".eslint-release-info.json", "utf8"),
	);
