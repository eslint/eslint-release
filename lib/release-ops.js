...
function calculateReleaseFromGitLogs(currentVersion, plogs, prereleaseId) {
    ...
    function generateChangelogLine(log) {
        return log.raw.replace(/^\* ([0-9a-f]{40})/, (_, hash) => `* [\`${hash.slice(0, 7)}\`](https://github.com/${repository}/commit/${hash})`);
    }
    const releaseInfo = {
...
function publishReleaseToGitHub(releaseInfo) {
    ...
    return repo.createRelease({
        tag_name: tag, // eslint-disable-line
