...
describe("ReleaseOps", () => {
	describe("getPrereleaseVersion()", () => {
		[
			...
			it(`(with ${version}, ${prereleaseId}, ${releaseType}, ${expected}) should return the correct next version`, () => {
				const result = ReleaseOps.getPrereleaseVersion(
					version,
					prereleaseId,
					releaseType,
				);
	...
	describe("calculateReleaseFromGitLogs()", () => {
		it("should create a patch release when only bug fixes are present", () => {
			const logs = [
				...
				releaseInfo = ReleaseOps.calculateReleaseFromGitLogs(
					"1.0.0",
					logs,
				);
	...
	describe("writeChangelog", () => {
		...
		it("creates a changelog", () => {
			...
			assert.strictEqual(
				newChangelog,
				`v1.0.0-alpha.0 - February 15, 2024\n\n${rawChangelog}\n\n`,
			);
		});
